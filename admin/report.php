...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        ...
        if ($action === 'add') {
            $sql = "INSERT INTO Reports (ReportType, ReportDetails, Analysis) VALUES (?, ?, ?)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("sss", $ReportType, $ReportDetails, $Analysis);
            $stmt->execute();
            $stmt->close();
        
        } elseif ($action === 'edit') {
            $ReportID = $_POST['ReportID'];
            $ReportDate = date("Y-m-d H:i:s");  // Mengatur waktu saat ini sebagai ReportDate
            $sql = "UPDATE Reports SET ReportType = ?, ReportDetails = ?, Analysis = ?, ReportDate = ? WHERE ReportID = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssssi", $ReportType, $ReportDetails, $Analysis, $ReportDate, $ReportID);
            $stmt->execute();
            $stmt->close();
        }
    }
...
<body>
    ...
    <div class="navbar">
        <ul>
            ...
            <li><a href="report.php" class="active"><i class="fas fa-chart-bar"></i> Report</a></li>
            <li>
        ...
        <table id="reportTable">
            ...
            ?>
                <tr id="row-<?php echo $row['ReportID']; ?>">
                    ...
                    <td>
                        <a href="#" onclick="openEditModal(<?php echo $row['ReportID']; ?>)"><i class="fas fa-edit"></i></a> |
                        <a href="#" onclick="confirmDelete(<?php echo $row['ReportID']; ?>)"><i class="fas fa-trash-alt"></i></a>
                    </td>
                </tr>
            <?php endwhile; ?>
        </table>
    ...
    <div id="addModal" class="modal">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="addForm" action="report.php" method="POST">
                    <input type="hidden" name="action" value="add">
                    <label for="ReportType">Report Type:</label>
                    <input type="text" id="addReportType" name="ReportType" required>
                    <label for="ReportDetails">Report Details:</label>
                    <textarea id="addReportDetails" name="ReportDetails" required></textarea>
                    <label for="Analysis">Analysis:</label>
                    <textarea id="addAnalysis" name="Analysis" required></textarea>
    ...
    <div id="editModal" class="modal">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="editForm" action="report.php" method="POST">
                    <input type="hidden" name="action" value="edit">
                    <input type="hidden" id="editReportID" name="ReportID">
                    <label for="editReportType">Report Type:</label>
                    <input type="text" id="editReportType" name="ReportType" required>
                    <label for="editReportDetails">Report Details:</label>
                    <textarea id="editReportDetails" name="ReportDetails" required></textarea>
                    <label for="editAnalysis">Analysis:</label>
                    <textarea id="editAnalysis" name="Analysis" required></textarea>
                    <div class="modal-footer">
    ...
    <script>
        ...
        function openEditModal(reportID) {
            ...
            const analysis = row.cells[4].innerText;

            document.getElementById('editReportID').value = reportID;
            document.getElementById('editReportType').value = reportType;
            document.getElementById('editReportDetails').value = reportDetails;
            document.getElementById('editAnalysis').value = analysis;
        ...
        function confirmDelete(reportID) {
            if (confirm('Are you sure you want to delete this report?')) {
                window.location.href = 'report.php?action=delete&id=' + reportID;
            }
        }
    </script>
</body>
