...
// Fungsi untuk mengambil data notifikasi berdasarkan ID
function fetchNotification($id) {
    global $conn;
    $query = "SELECT * FROM EvaluationReports WHERE EvaluationID = ?";
...
// Proses tambah atau edit notifikasi
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        ...
        if ($action === 'add') {
            $sql = "INSERT INTO EvaluationReports (NotificationID, Message, Description, EvaluationDate) VALUES (?, ?, ?, NOW())";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("iss", $NotificationID, $Message, $Description);
            $stmt->execute();
            $stmt->close();

        } elseif ($action === 'edit') {
            $sql = "UPDATE EvaluationReports SET NotificationID = ?, Message = ?, Description = ?, EvaluationDate = NOW() WHERE EvaluationID = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("issi", $NotificationID, $Message, $Description, $EvaluationID);
            $stmt->execute();
            $stmt->close();
        }
    }
...
// Proses penghapusan notifikasi
if (isset($_GET['action'])) {
    if ($_GET['action'] == 'delete' && isset($_GET['id'])) {
        $id = $_GET['id'];
        $deleteQuery = "DELETE FROM EvaluationReports WHERE EvaluationID = ?";
    ...
    <div class="container">
        ...
        <table>
            ...
            <tbody>
                <?php
                include 'koneksi.php';
                $result = $conn->query("SELECT * FROM EvaluationReports");
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<td>";
                        echo '<a href="#" onclick="openEditModal(' . $row['EvaluationID'] . ')"><i class="fas fa-edit"></i></a>';
                        echo '<a href="#" onclick="confirmDelete(' . $row['EvaluationID'] . ')"><i class="fas fa-trash-alt"></i></a>';
                        echo "</td>";
                        echo "</tr>";
                    }
                } else {
        ...
        <button class="add-btn" onclick="showModal('add')">Add Notification</button>
    </div>
    ...
    <script>
        ...
        function fetchNotification(id) {
            fetch(`notification.php?action=get&id=${id}`)
                ...
                .then(data => {
                    document.getElementById('EvaluationID').value = data.EvaluationID;
